...
entry:                      ;内核入口
    ...
    ltr [ecx+0x18]              ;特权指令(16位可见+高速缓冲)，非TSS选择子异常
    lldt [ecx+0x10]             ;特权指令(16位可见+高速缓冲)，非LDT选择子异常

    mov eax,[ecx+0x44]          ;切换到应用程序头部
    mov ds,eax

    ;模仿调用门返回
    push dword [0x08]           ;调用前ss       0x0000001f
    push dword 0                ;调用前esp      0x00000000
    ;push ...                   ;调用前参数
    push dword [0x14]           ;调用前cs       0x0000000f
    push dword [0x10]           ;调用前eip      0x00000000
